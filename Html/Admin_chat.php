...
<body>
  ...
  <form method="get" class="filters" id="filterForm">
    <label>Search:
      <input type="search" name="q" placeholder="Search message text" value="<?= htmlspecialchars($q) ?>"
    ...
    <label>Source:
      <select name="source" onchange="document.getElementById('filterForm').submit()">
        <option value="all" <?php if ($source === 'all')
          echo ' selected'; ?>>All</option>
        <option value="user" <?php if ($source === 'user')
          echo ' selected'; ?>>User</option>
        <option value="bot" <?php if ($source === 'bot')
    ...
    <label>Role:
      <select name="role" onchange="document.getElementById('filterForm').submit()">
        <option value="all" <?php if ($roleFilter === '' || $roleFilter === 'all')
          echo ' selected'; ?>>All</option>
        <?php foreach ($roles as $r):
          $rv = htmlspecialchars($r);
  ...
  <div class="pagination" style="margin-top:12px;display:flex;gap:8px;align-items:center">
    ...
    for ($p = $start; $p <= $end; $p++):
      ...
      <?php else: ?>
        <a
          href="?page=<?php echo $p; ?>&source=<?php echo urlencode($source); ?>&role=<?php echo urlencode($roleFilter); ?>&q=<?php echo urlencode($q); ?>"><?php echo $p; ?></a>
      <?php endif; ?>
    <?php endfor; ?>

    <?php if ($page < $totalPages): ?>
      <a
        href="?page=<?php echo $page + 1; ?>&source=<?php echo urlencode($source); ?>&role=<?php echo urlencode($roleFilter); ?>&q=<?php echo urlencode($q); ?>">Next
        &raquo;</a>
    <?php endif; ?>
  </div>
